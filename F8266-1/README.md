...
14. **数据上报 + 远程控制**：每 2s（`NET_REPORT_MS`）打包传感数据为 JSON 帧上报主机——温湿度/光照/土壤/eCO₂/TVOC（**不含 MQ-2**，该传感器仅本机使用），无数据字段填 `null`（`@{"t":25.3,"h":56.0,"lux":1234,"soil":1,"co2":800,"tvoc":12}/`）；接收主机下发命令并执行——`PUMP ON/OFF`、`FAN FWD/REV/STOP`、`SERVO 0~180`（`onHostCommand()` 内解析，未知命令仅打印）；帧长上限 `NET_FRAME_MAX_LEN` 64→128。
