...
**命名约定**：设备身份、STA 主机名、配置热点名统一使用 `F8266-1`，与项目目录同名。该身份对应仓库 [esp32-1](../esp32-1/) 主机 6 路 TCP 从机端口中的 `F8266-x` 命名体系——F8266-1 接入主机 **8000** 端口。主机 IP 默认支持**动态发现**（监听主机 UDP 广播）；回退地址 `192.168.1.100` 仅在网络中无主机宣告时兜底（改主机 IP / 端口 / 广播参数只动 [net_config.h](src/net_config.h)，须保证与从机同一网段）。命令词表已定义（`PUMP`/`FAN`/`SERVO`，见第 5/14 项），`onHostCommand()` 已解析执行，未知命令仅串口打印。
...
5. **主机 TCP 链路**：联网后自动作为 TCP 客户端连主机 `:8000`，连上即发上线帧 `@F8266-1 online/`（主机网页对应端口变绿）；在线期间接收主机 `@…/` 命令帧并回调 `onHostCommand()` 解析执行执行器命令（`PUMP ON/OFF`、`FAN FWD/REV/STOP`、`SERVO 0~180`），未知命令仅串口打印；主机离线自动按重试间隔重连，重连成功重新上报上线；WiFi 掉线期间链路自动暂停，恢复后重连。
...
13. **执行实体（水泵/风扇/舵机，首个执行层）**：`actuator_ctrl` 模块，控制入口已接主机命令词表（见第 14 项）；**上电安全态**——泵关（GPIO0 低）、风扇停（GPIO13/2 双高）、舵机回中位 90°（GPIO12，Servo 库 50Hz）；**触发极性**——水泵**高触发**（高=开，GPIO0 烧录拉低=关，安全）、风扇双路**低触发**（低=开）+ 正反转**软件互锁**（开任一路前强制断开另一路，杜绝两路同触导致驱动级电源直通短路）；**上电自检**——初始化完成后逐个激活执行器 0.5s（泵开→扇正转→扇反转→舵机摆 180°→回中）供人工检查接线/动作；舵机 `servoSet(0~180°)` 自动夹取；OLED 行 5 显示 `P:OFF F:OFF S:90` 实时反映执行器状态，全部动作串口打印。
14. **数据上报 + 远程控制**：每 2s（`NET_REPORT_MS`）打包传感数据 + **执行器实时状态**为 JSON 帧上报主机——温湿度/光照/土壤/eCO₂/TVOC（**不含 MQ-2**，该传感器仅本机使用），无数据字段填 `null`；并含 `pump`(0/1)、`fan`(0停/1正转/2反转)、`servo`(0~180°) 三字段，主机网页执行器卡据此实时显示当前态（`@{"t":25.3,"h":56.0,"lux":1234,"soil":1,"co2":800,"tvoc":12,"pump":0,"fan":1,"servo":90}/`）；接收主机下发命令并执行——`PUMP ON/OFF`、`FAN FWD/REV/STOP`、`SERVO 0~180`（`onHostCommand()` 内解析，未知命令仅打印）；帧长上限 `NET_FRAME_MAX_LEN` 64→128。
...
**预期现象**：烧录上电后，OLED 即点亮标题行（`F8266-1 SmartFarm`），串口打印 `[oled] SSD1306 已激活`——若屏不亮，检查接线并把 `oled_ctrl.cpp` 的 `OLED_I2C_ADDR` 在 0x3C/0x3D 间切换（OLED 失败不影响后续联网）。SHT30 接好时串口打印 `[sht] SHT30 已检测到（0x44）`，数秒后 OLED 信息区行 0 显示 `T:xx.xC H:xx%`（每 2s 更新读数、串口同步打印）；未接/故障时该行显示 `SHT30 ERR`（每 10s 自动重扫，接上即恢复）。GY-30 接好时串口打印 `[gy30] GY-30(BH1750) 已检测到（0x23）`，行 1 显示 `L:xxxxlux`（手机手电筒照射可见读数明显变化）；未接/故障时显示 `GY30 ERR`（同上自愈）。MQ-2 上电后行 2 先显示 `MQ2 HEAT`（3 分钟预热），预热完成后显示 `G:xx`（清洁空气基线以现场实测为准，用于标定 `MQ2_ALARM_RAW` 阈值）；用打火机未点火放气靠近传感器，读数迅速上升超过阈值即显示 `MQ2 GAS!` 并串口打印 `<<< GAS ALARM`；若预热后长期 `G:0`（AO 断线/未共地）或长期贴满量程（分压失效）显示 `MQ2 ERR`。土壤传感器探头插干土行 3 显示 `SOIL:DRY`，浇水 1s 后切换 `SOIL:WET`（串口打印「判定切换」；浇水边缘来回跳变时状态保持不翻转；当前模块实测低电平=湿润，极性已按此设定）。SGP30 上电行 4 先显示 `SGP30 INIT`（15s 暖机），随后显示 `C:xxxppm V:xxppb`（对传感器哈气/酒精棉靠近，eCO₂ 与 TVOC 显著上升；清洁环境 eCO₂ 常见 400~600ppm 基线）。执行器上电先进入安全态，随即**上电自检**——泵开 0.5s、风扇正转 0.5s、风扇反转 0.5s、舵机摆到 180° 再回中（串口依次打印 `[act] 水泵 开`/`风扇 正转`/`风扇 反转`/`舵机 180°`），供人工核对每个执行器接线与动作；自检结束后行 5 显示 `P:OFF F:OFF S:90`（安全态）。当前控制入口（主机命令/自动逻辑）尚未接入，自检后执行器保持安全态属预期。随后若环境存在 `ESP-TEST` 网络，串口打印连上后的 IP；若不存在，设备变为配置热点 `F8266-1`，手机连上后浏览器访问 <http://192.168.4.1> 可配置网络。连上默认网络后，只要主机 esp32-1 在线（DHCP 动态 IP 亦可，从机会收到其广播宣告自动切换目标地址；串口会打印「广播发现主机: `<IP>`」），串口即打印「上报上线」，主机网页 8000 端口卡变绿显示「F8266-1 已上线」（主机与从机须在同一局域网网段，且 `NET_HOST_ANNOUNCE_PORT` 与主机 `HOST_ANNOUNCE_PORT` 一致）。
...
| 日期 | 变更内容 |
| --- | --- |
| 2026-09-08 | 执行器状态回传：传感 JSON 上报追加 `pump/fan/servo` 三字段，主机网页执行器卡实时显示从机当前态 |
| 2026-09-08 | 数据上报 + 远程控制：每 2s 打包传感 JSON（温湿度/光照/土壤/eCO₂/TVOC，不含 MQ-2）上报主机；`onHostCommand` 解析执行器命令（PUMP/FAN/SERVO）；`NET_FRAME_MAX_LEN` 64→128，编译通过 |
| 2026-09-08 | 执行器极性调整 + 上电自检：水泵改**高触发**（高=开，GPIO0 烧录拉低=关，彻底消除烧录误开喷水风险）、风扇保持低触发；新增 `selfTest()` 上电自检——初始化完成后逐个激活执行器 0.5s（泵→扇正→扇反→舵机摆 180°→回中）供人工检查；编译通过 |
| 2026-09-08 | 执行实体接入：新增 `actuator_ctrl` 模块（水泵 GPIO0 / 风扇正反转 GPIO13·2 / 舵机 GPIO12），上电安全态 + 风扇正反转软件互锁，OLED 行 5 状态上屏（信息区 6 行满员），控制入口待接，编译通过 |
