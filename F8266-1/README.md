...
13. **执行实体（水泵/风扇/舵机，首个执行层）**：`actuator_ctrl` 模块，控制入口已接主机命令词表（见第 14 项）；**上电安全态**——泵关（GPIO0 低）、风扇停（GPIO13/2 双高）、舵机回中位 90°（GPIO12，Servo 库 50Hz）；**触发极性**——水泵**高触发**（高=开，GPIO0 烧录拉低=关，安全）、风扇双路**低触发**（低=开）+ 正反转**软件互锁**（开任一路前强制断开另一路，杜绝两路同触导致驱动级电源直通短路）；**上电自检**——初始化完成后逐个激活执行器 0.5s（泵开→扇正转→扇反转→舵机摆 180°→回中）供人工检查接线/动作；**断连保护**——与主机连接断开（TCP 断/WiFi 断）即 `allStop()` 全停（泵关+风扇停+舵机回中位），重连后不自动恢复（需主机重新下发命令）；**重连补报**——重连成功立即补发一次设备状态（传感+执行器实时态），让主机网页快速同步（断线期间执行器已全停，主机旧显示可能不符）；舵机 `servoSet(0~180°)` 自动夹取；OLED 行 5 显示 `P:OFF F:OFF S:90` 实时反映执行器状态，全部动作串口打印。
...
| 日期 | 变更内容 |
| --- | --- |
| 2026-09-08 | 断连保护 + 重连补报：新增 `actuator_ctrl::allStop()`，main 检测与主机连接在线→断开（TCP/WiFi 断）即全停执行器（泵关+扇停+舵机回中位 90°）；断开→重连成功即立即补发一次设备状态（传感+执行器实时态）；重连后不自动恢复需主机重发命令，编译通过 |
