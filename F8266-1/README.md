...
14. **数据上报 + 远程控制**：每 2s（`NET_REPORT_MS`）打包传感数据 + **执行器实时状态**为 JSON 帧上报主机——温湿度/光照/土壤/eCO₂/TVOC（**不含 MQ-2**，该传感器仅本机使用），无数据字段填 `null`；并含 `pump`(0/1)、`fan`(0停/1正转/2反转)、`servo`(0~180°) 三字段，主机网页执行器卡据此实时显示当前态（`@{"t":25.3,"h":56.0,"lux":1234,"soil":1,"co2":800,"tvoc":12,"pump":0,"fan":1,"servo":0}/`，0=关棚 180=开棚）；接收主机下发命令并执行——`PUMP ON/OFF`、`FAN FWD/REV/STOP`、`SERVO 0~180`（`onHostCommand()` 内解析，未知命令仅打印）；帧长上限 `NET_FRAME_MAX_LEN` 64→128。
...
**预期现象**：烧录上电后，OLED 即点亮标题行（`F8266-1 SmartFarm`），串口打印 `[oled] SSD1306 已激活`——若屏不亮，检查接线并把 `oled_ctrl.cpp` 的 `OLED_I2C_ADDR` 在 0x3C/0x3D 间切换（OLED 失败不影响后续联网）。SHT30 接好时串口打印 `[sht] SHT30 已检测到（0x44）`，数秒后 OLED 信息区行 0 显示 `T:xx.xC H:xx%`（每 2s 更新读数、串口同步打印）；未接/故障时该行显示 `SHT30 ERR`（每 10s 自动重扫，接上即恢复）。GY-30 接好时串口打印 `[gy30] GY-30(BH1750) 已检测到（0x23）`，行 1 显示 `L:xxxxlux`（手机手电筒照射可见读数明显变化）；未接/故障时显示 `GY30 ERR`（同上自愈）。MQ-2 上电后行 2 先显示 `MQ2 HEAT`（3 分钟预热），预热完成后显示 `G:xx`（清洁空气基线以现场实测为准，用于标定 `MQ2_ALARM_RAW` 阈值）；用打火机未点火放气靠近传感器，读数迅速上升超过阈值即显示 `MQ2 GAS!` 并串口打印 `<<< GAS ALARM`；若预热后长期 `G:0`（AO 断线/未共地）或长期贴满量程（分压失效）显示 `MQ2 ERR`。土壤传感器探头插干土行 3 显示 `SOIL:DRY`，浇水 1s 后切换 `SOIL:WET`（串口打印「判定切换」；浇水边缘来回跳变时状态保持不翻转；当前模块实测低电平=湿润，极性已按此设定）。SGP30 上电行 4 先显示 `SGP30 INIT`（15s 暖机），随后显示 `C:xxxppm V:xxppb`（对传感器哈气/酒精棉靠近，eCO₂ 与 TVOC 显著上升；清洁环境 eCO₂ 常见 400~600ppm 基线）。执行器上电先进入安全态（舵机 0°=关棚），随即**上电自检**——泵开 0.5s、风扇正转 0.5s、风扇反转 0.5s、舵机摆到 180°（开棚演示）再回 0° 关棚（串口依次打印 `[act] 水泵 开`/`风扇 正转`/`风扇 反转`/`舵机 180°`），供人工核对每个执行器接线与动作；自检结束后行 5 显示 `P:OFF F:OFF S:0`（关棚安全态）。随后若环境存在 `ESP-TEST` 网络，串口打印连上后的 IP；若不存在，设备变为配置热点 `F8266-1`，手机连上后浏览器访问 <http://192.168.4.1> 可配置网络。连上默认网络后，只要主机 esp32-1 在线（DHCP 动态 IP 亦可，从机会收到其广播宣告自动切换目标地址；串口会打印「广播发现主机: `<IP>`」），串口即打印「上报上线」，主机网页 8000 端口卡变绿显示「F8266-1 已上线」（主机与从机须在同一局域网网段，且 `NET_HOST_ANNOUNCE_PORT` 与主机 `HOST_ANNOUNCE_PORT` 一致）。
...
| 日期 | 变更内容 |
| --- | --- |
| 2026-09-08 | 舵机业务语义改为**大棚开合**：0°=关棚 / 180°=开棚；上电安全位/`allStop()`/自检回位由原 90° 中位改为 **0°(关棚)**，注释与串口文案同步 |
| 2026-09-08 | 断连保护 + 重连补报：新增 `actuator_ctrl::allStop()`，main 检测与主机连接在线→断开（TCP/WiFi 断）即全停执行器（泵关+扇停+舵机 0°关棚）；断开→重连成功即立即补发一次设备状态（传感+执行器实时态）；重连后不自动恢复需主机重发命令，编译通过 |
