...
// 接线（全部低电平触发型驱动模块；★执行器电源独立供电并共地）：
//   水泵继电器/MOS IN → GPIO0（低=开泵；★烧录脚：上传固件时被拉低→水泵误开喷水，
//   烧录前务必断水断电；且 GPIO0 上电需高，须确认泵模块 IN 无下拉电阻否则无法启动）
//   风扇双路驱动 IN1(正转)/IN2(反转) → GPIO13 / GPIO2（GPIO2 为 boot 脚上电需高，
//   低触发设备上电默认关=安全）
//   舵机信号线（SG90 橙线）→ GPIO12（50Hz PWM）
// 安全设计：
//   ① 上电/初始化即进入安全态：泵关、风扇停、舵机回中位 90°；
...
class ActuatorCtrl {
 public:
  ...
  // 初始化全部 GPIO 并进入安全态（泵关/风扇停/舵机 90°）；返回 true（无探测概念）
  bool begin();
