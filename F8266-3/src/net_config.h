...
// ===================== F8266-3 网络配置（集中在此修改）=====================
// 设备统一命名 F8266-3：STA 主机名与配置热点名均取 NET_DEVICE_NAME。
...
#define NET_DEVICE_NAME        "F8266-3"
...
// ===================== 主机（esp32-1）TCP 通信参数 =====================
// 本从机(F8266-3)作为 TCP 客户端连主机 NET_HOST_IP:NET_HOST_PORT，连上即上报上线。
...
#define NET_HOST_PORT     8002             // 本从机(F8266-3)对应主机的监听端口
...
// 注：传感数据 JSON 上报约 80~100 字符，故取 128（主机端解析上限 256，兼容）。
#define NET_FRAME_MAX_LEN 128

// 传感数据 JSON 上报周期（毫秒）；2s 上报一次到主机（不含 MQ-2，见 main.cpp）。
