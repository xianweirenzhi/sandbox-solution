/**
 * ESP32-S3 智慧农场控制中心(六从机可视化主机)入口
 *
 * 流程:
 *   1. 上电以 STA 模式连接路由器 ESP-TEST(最长 2 分钟)
 *   2. 连接成功 -> 正常 STA 模式,同局域网访问设备 IP 打开控制中心页
 *   3. 连接超时 -> 自动切换 AP 直连模式(热点 ESP32-Direct-2),
 *      手机连接该热点后访问 192.168.4.1 打开同一页面
 *
 * 模块划分(便于维护与扩展):
 *   config.h         全部可调参数
 ...
 *   auto_ctrl.*      智能大棚自动控制:每端口独立实例(阈值按端口存 NVS)
 *   web_ui.*         网页服务 v2:响应式仪表盘(总览/详情/日志)
 */
#include <Arduino.h>
#include <WiFi.h>
