...
// 土壤判定按端口双模式(config.h PORT_SOIL_RAW):数字两态口 0=干/1=湿;
// 原始值口(如 F8266-2)读数 0~1023 低=湿高=干,≥soilDry 判干/≤soilWet 判湿,之间保持(滞回)。
...
struct Config {
  ...
  float soilWet = 0.0f;    // 土壤湿阈值,原始值 0~1023(仅原始值端口:读数≤此值=湿润解除浇水)
  float soilDry = 0.0f;    // 土壤干阈值,原始值 0~1023(仅原始值端口:读数≥此值=过干触发浇水)
};
