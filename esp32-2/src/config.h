...
// 土壤浇水阈值(仅土壤原始值端口生效,见下方 PORT_SOIL_RAW;单位=滤波后原始 ADC 值
// 0~1023,低=湿 高=干):读数 ≥ 干阈值 → 判过干触发水泵;≤ 湿阈值 → 判湿润解除;
// 两者之间保持(滞回)。★实测 F8266-2 模块摆幅仅 ~40~90 counts,默认 360/560 不会
// 触发浇水(安全),须按网页读数现场标定后修改。
#define AC_SOIL_WET    360.0f    // 湿阈值(≤ 此值=湿润解除浇水)
#define AC_SOIL_DRY    560.0f    // 干阈值(≥ 此值=过干触发浇水)
