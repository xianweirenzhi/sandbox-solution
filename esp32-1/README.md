...
ESP32-S3 主机设备项目：优先以 **STA 模式**（**固定 IP**）连接路由器；若 2 分钟内无法连接，**自动切换 AP 直连兜底模式**。作为从机通信的**主机**，开启 **6 个连续 TCP 端口**（8000~8005）与从机（F8266-x）通信。网页提供「精简设备状态卡 + 2×3 六端口收发窗口」，可实时查看各端口收发报文、从机上线状态，并直接向从机发送报文。
...
- [x] STA 模式直连指定路由器（**固定 IP**，见 `config.h`），串口输出连接验证信息
- [x] 2 分钟连接超时后自动切换 AP 模式（热点 `ESP32-Direct`），手机直连可访问设备网页
- [x] 6 个连续 TCP 端口（8000~8005）作为主机监听从机连接，行式报文收发
...
└── src/
    ...
    ├── config.h          ★ 全部可调参数（WiFi、固定 IP、超时、AP 网段、端口）
    ├── wifi_service.h/.cpp   WiFi 管理：STA 优先(固定 IP) → 超时切 AP
...
| `STA_STATIC_IP` | `192.168.4.200` | STA 固定 IP（**须与路由器同网段**） |
| `STA_GATEWAY` | `192.168.4.1` | 网关（路由器 IP） |
| `STA_NETMASK` | `255.255.255.0` | 子网掩码 |
| `STA_DNS` | `192.168.4.1` | DNS（与网关一致即可） |
...
| 路由 | 方法 | 说明 |
|---|---|---|
| `/` | GET | 主机控制台页面（HTML，存于 Flash/PROGMEM）：精简状态卡 + 2×3 端口收发窗口 |
| `/api/status` | GET | 设备状态 JSON |
| `/api/ports` | GET | 6 个从机通信端口状态 JSON（数组） |
| `/api/send` | POST | 向指定端口发送报文，参数 `port`（端口号）、`text`（内容，自动补 `\n`） |
| 其他 | — | 返回 404 |
...
- **报文格式**：行式（以 `\n` 结尾）；主机下发的报文自动补 `\n`
- **上线**：从机连接后发送 `F8266-x online`（x 为从机编号），主机网页对应端口卡标记「**F8266-x 已上线**」（绿点），日志区出现 ★ 记录
...
| # | 场景 | 操作 | 预期 |
|---|---|---|---|
| 1 | STA 正常连接（固定 IP） | 开启 `ESP-TEST` 热点后上电 | 串口打印固定 IP（默认 `192.168.4.200`），浏览器访问该 IP 出现控制台页 |
...
| 6 | 从机离线 | 断开调试助手连接 | 端口卡变「无从机」，日志出现 ○ 离线记录 |
| 7 | 串口无输出 | — | 检查串口波特率为 115200；本板使用 USB CDC，换 USB 线/口重试 |
...
- **设置了固定 IP 后 STA 反而连不上**：固定 IP 必须与路由器同网段、网关须为路由器 IP；手机热点网段常见为安卓 `192.168.43.x`、iPhone `172.20.10.x`，请按实际网段修改 `config.h` 中的 `STA_STATIC_IP/STA_GATEWAY/STA_DNS`
