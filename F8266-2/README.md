...
基于 PlatformIO + Arduino 的 ESP8266（Adafruit HUZZAH ESP8266，4MB Flash）测试项目。**本项目由 [F8266-1](../F8266-1/) 于 2026-09-08 完整克隆而来**，业务能力与 F8266-1 同构（智慧农场状态节点），差异仅三处：设备身份 `F8266-2`、主机端口 8001、构建环境名。**当前状态：编译通过，待上板联调**（硬件按 F8266-1 同款接线逐步接回，未接的传感器会显示故障并热插拔自愈，不影响运行）。
...
**业务定位（智慧农场·同构第二节点）**：与 F8266-1 相同——智慧农场子系统的**状态节点**，接入传感子系统① SHT30 温湿度、② GY-30 光照、③ MQ-2 可燃气体、④ 土壤湿度、⑤ SGP30 空气质量（eCO₂/TVOC）+ 执行实体（水泵/风扇/舵机），读数/故障/执行器状态实时上屏（信息区 6 行满员）并周期上报主机。
...
代码按职责拆分到独立模块，`main.cpp` 仅做编排（详见"目录结构"），与 F8266-1 同构：
...
7. **OLED 状态屏（状态可视化层）**：0.96" SSD1306（I2C：SDA=GPIO4、SCL=GPIO5，地址 0x3C）开机即点亮，标题行（设备名 + SmartFarm 合并一行）；下方**信息区 6 行**由各子系统经 `setInfoLine(row, ...)` 写入状态文本（每子系统一行，通用接口，行内容变化才重绘）（文字统一 ASCII/英文，暂不引入中文字库）。
...
13. **执行实体（水泵/风扇/舵机，首个执行层）**：`actuator_ctrl` 模块，控制入口已接主机命令词表（见第 14 项）；★**业务语义：舵机=大棚——0°=关棚 / 180°=开棚，安全位取 0°(关棚)**；**上电安全态**——泵关（GPIO0 低）、风扇停（GPIO13/2 双高）、舵机回 0° 关棚（GPIO12，Servo 库 50Hz）；**触发极性**——水泵**高触发**（高=开，GPIO0 烧录拉低=关，安全）、风扇双路**低触发**（低=开）+ 正反转**软件互锁**（开任一路前强制断开另一路，杜绝两路同触导致驱动级电源直通短路）；**上电自检**——初始化完成后逐个激活执行器 0.5s（泵开→扇正转→扇反转→舵机摆 180°(开棚演示)→回 0°关棚）供人工检查接线/动作；**断连保护**——与主机连接断开（TCP 断/WiFi 断）即 `allStop()` 全停（泵关+风扇停+舵机 0°关棚），重连后不自动恢复（需主机重新下发命令）；**重连补报**——重连成功立即补发一次设备状态（传感+执行器实时态），让主机网页快速同步（断线期间执行器已全停，主机旧显示可能不符）；舵机 `servoSet(0~180°)` 自动夹取；OLED 行 5 显示 `P:OFF F:OFF S:0`(关棚) 实时反映执行器状态，全部动作串口打印。
14. **数据上报 + 远程控制**：每 2s（`NET_REPORT_MS`）打包传感数据 + **执行器实时状态**为 JSON 帧上报主机——温湿度/光照/土壤/eCO₂/TVOC（**不含 MQ-2**，该传感器仅本机使用），无数据字段填 `null`；并含 `pump`(0/1)、`fan`(0停/1正转/2反转)、`servo`(0~180°) 三字段，主机网页执行器卡据此实时显示当前态（`@{"t":25.3,"h":56.0,"lux":1234,"soil":1,"co2":800,"tvoc":12,"pump":0,"fan":1,"servo":0}/`，0=关棚 180=开棚）；接收主机下发命令并执行——`PUMP ON/OFF`、`FAN FWD/REV/STOP`、`SERVO 0~180`（`onHostCommand()` 内解析，未知命令仅打印）；帧长上限 `NET_FRAME_MAX_LEN`（128 字符）。
...
**预期现象**：烧录上电后，OLED 即点亮标题行（`F8266-2 SmartFarm`）——未接外设时各传感行显示对应 `ERR`（每 10s 自动重扫，接上即恢复），执行器自检无负载则仅串口打印动作。完整现象（含各传感器读数与执行器动作）与 F8266-1 一致，见 [F8266-1 README「预期现象」](../F8266-1/README.md)。联网后若主机 esp32-1 在线，串口打印「上报上线」，主机网页 **8001** 端口卡变绿显示「**F8266-2 已上线**」（主机与从机须在同一局域网网段，且 `NET_HOST_ANNOUNCE_PORT` 与主机 `HOST_ANNOUNCE_PORT` 一致）。
