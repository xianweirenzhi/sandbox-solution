...
// ★光照现场标定倍率：本模块 MAX44009 实测读数恒偏低（兼容片灵敏度/模块窗口衰减的
// 固定倍率偏差）。已按 F8266-1(BH1750) 并排对照标定：环境光本机原始 ~5.4lux ↔ 参考
// ~75lux，倍率 ≈ 14。再精调方法：同照度下倍率 = 参考值 / 本机原始值（原始值见
// 开机前 10 次 raw 打印），改此宏即可。
#define GY39_LUX_CAL     14.0f
