...
//
// 两种接法固件自适应（VCC→3V3、GND→GND 不变）：
//   ① UART 模式（出厂默认，★免焊推荐）：模块 CT(=TX)→PERIPH_GY39_UART_RX(GPIO14)，
//      DR 不接；9600bps 主动持续发帧：5A 5A <type> <len> <data[len]> <sum>
//      （0x15=光照 4B、0x45=温度2B+气压4B+湿度2B+海拔2B，大端 /100，累加和校验）
//   ② I2C 模式（须 S0 焊桥接 GND）：CT→SCL、DR→SDA，7 位地址 0x5B，
//      直读 0x00~0x0D 数据寄存器（14B 大端：lux/T/P/HUM）
// 探测策略：后台始终解析 UART 字节流 + 周期探测 I2C 0x5B，哪边先出有效数据用哪边；
//   UART 帧流停止超时自动降级回探测（支持换接线热切换）。
//
